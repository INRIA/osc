...
module UnobtrusiveDatePicker
   class Engine < Rails::Engine
    initializer "unobtrusive_date_picker" do
   ...
   module UnobtrusiveDatePickerHelper
      ...
      def unobtrusive_date_picker(object_name, method, options = {})
         (ActionView::Helpers::InstanceTag.new(object_name, method, self,options.delete(:object)).to_datepicker_date_select_tag(options)).html_safe()
      end
      ...
      def unobtrusive_datetime_picker(object_name, method, options = {})
         ActionView::Helpers::InstanceTag.new(object_name, method, self, options.delete(:object)).to_datepicker_datetime_select_tag(options)
      end
      ...
      def datepicker_select_year(date, options = {})
         ...
         start_year, end_year = (options[:start_year] || y-10), (options[:end_year] || y+10)
   ...
   module InstanceTag # :nodoc: all
      ...
      def datepicker_options_with_prefix(position, options)
         ...
         if options[:id].nil?
           ...
           options[:id_prefix] = "#{@object_name}_#{@method_name}_"+options[:id]
         end
         if options[:string]
