...
class DepenseNonVentileeFacture < ActiveRecord::Base
  belongs_to :depense_non_ventilee
  ...
  validates_date  :date, 
                  :before => Proc.new{ |r| r.depense_non_ventilee.ligne.date_fin_depenses+ 1.day},
                  :after => Proc.new{ |r| r.depense_non_ventilee.ligne.date_debut- 1.day}, 
  ...
  def update_depense
    ...
    self.depense_non_ventilee.updated_at = Time.now
    self.depense_non_ventilee.save!
  end

  def update_depense_destroy
    self.depense_non_ventilee.update_dates_min_max
    self.depense_non_ventilee.updated_at = Time.now
    self.depense_non_ventilee.save!
  end
  ...
  def taux_cout_projet
    ...
    organisme_gestionnaire_id = self.depense_non_ventilee.ligne.organisme_gestionnaire.id
