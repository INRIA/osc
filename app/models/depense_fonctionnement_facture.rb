...
class DepenseFonctionnementFacture < ActiveRecord::Base
  belongs_to :depense_fonctionnement
  ...
  validates_date  :date, 
                  :before => Proc.new{ |r| r.depense_fonctionnement.ligne.date_fin_depenses+ 1.day},
                  :after => Proc.new{ |r| r.depense_fonctionnement.ligne.date_debut- 1.day}, 
  ...
  attr_accessible :date, :millesime, :numero_facture, :commentaire, :numero_mandat, :date_mandatement, :cout_ht, 
                  :taux_tva, :montant_htr, :justifiable, :rubrique_comptable_id, :code_analytique, :compte_budgetaire
  
  def update_depense
    self.updated_by = User.current_user.id
    self.depense_fonctionnement.updated_at = Time.now
    self.depense_fonctionnement.save!
  end

  def update_depense_destroy
    self.depense_fonctionnement.update_dates_min_max
    self.depense_fonctionnement.updated_at = Time.now
    self.depense_fonctionnement.save!
  end
  ...
  def taux_cout_projet
    ...
    organisme_gestionnaire_id = self.depense_fonctionnement.ligne.organisme_gestionnaire.id
