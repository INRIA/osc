...
class DepenseFonctionnement < ActiveRecord::Base
  belongs_to :ligne
  has_many :depense_fonctionnement_factures, :dependent => :destroy
	has_many :rubrique_comptables, :through => :depense_fonctionnement_factures
  ...
  attr_accessible :ligne_id, :reference, :date_commande, :fournisseur, :structure, :commentaire, 
                  :montant_engage, :intitule, :commande_solde, :tache, :eligible, :prestation_service,
                  :type_activite, :code_analytique, :compte_budgetaire, :code_projet
  ...
  def self.localized_human_attribute_name(attr)
    return case attr
      ...
      when "millesime"                       then "Date de service fait"
      when "depense_fonctionnement_factures" then "Facture"
      else attr.humanize
    end
  end
  ...
  def update_dates_min_max
    ...
    if self.depense_fonctionnement_factures.size != 0
      facture_date_max = [self.depense_fonctionnement_factures.maximum(:date),self.depense_fonctionnement_factures.maximum(:millesime)||self.date_commande].max || self.date_commande
      self.date_max = [facture_date_max, self.date_commande,millesime].max
      facture_date_min = [self.depense_fonctionnement_factures.minimum(:date),self.depense_fonctionnement_factures.minimum(:millesime)||self.date_commande].min || self.date_commande
  ...
  def montant_factures(include_taxes = 'htr', date_start = '1900-01-01', date_end = '4000-01-01')
    ...
    factures = self.depense_fonctionnement_factures.find(:all, :conditions => find_conditions)
    somme = 0
    for facture in factures
      date_facture = facture.millesime || facture.date
  ...
  def montant(date_start = '1900-01-01', date_end = '4000-01-01', type_bilan = "sommes_engagees", type_montant = "ttc")
    ...
    if type_bilan == "sommes_engagees"
      if self.commande_solde && (self.depense_fonctionnement_factures.size !=0)
        self.montant_factures(type_montant, date_start, date_end)
      elsif date_commande.to_date <= date_end.to_date && date_commande.to_date >= date_start.to_date
        self.montant_engage
      else
        0
      end
  ...
  def reporting_montant(date_start = '1900-01-01', date_end = '4000-01-01', type = 'facture_htr')
    if type == 'facture_htr'
      ...
      factures = self.depense_fonctionnement_factures.find(:all, options)
      if factures.nil?
        return 0
      else
  ...
  def self.reporting_total_depense(ids = [], options = {})
    ...
    prestation_service = options["prestation_service"] || false
    ...
    if type == "realisees"
      if type_activite == false
        conditions = ["ligne_id in (?) AND eligible = ? AND verrou = 'SI INRIA' AND prestation_service = ?", ids, eligible, prestation_service]
      else
