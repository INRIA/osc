...
class DepenseEquipement < ActiveRecord::Base
  belongs_to :ligne
  has_many :depense_equipement_factures, :dependent => :destroy
	has_many :rubrique_comptables, :through => :depense_equipement_factures
  ...
  def self.localized_human_attribute_name(attr)
    return case attr
      ...
      when "depense_equipement_factures" then "Facture"
      else attr.humanize
    end
  end
  ...
  def update_dates_min_max
    ...
    if self.depense_equipement_factures.size != 0
      facture_date_max = [self.depense_equipement_factures.maximum(:date),self.depense_equipement_factures.maximum(:millesime)||self.date_commande].max || self.date_commande
      self.date_max = [facture_date_max, self.date_commande,millesime].max
      facture_date_min = [self.depense_equipement_factures.minimum(:date),self.depense_equipement_factures.minimum(:millesime)||self.date_commande].min || self.date_commande
  ...
  def montant_factures(include_taxes = 'htr', date_start = '1900-01-01', date_end = '4000-01-01')
    ...
    factures = self.depense_equipement_factures.find(:all, :conditions => find_conditions)
    somme = 0
  ...
  def montant(date_start = '1900-01-01', date_end = '4000-01-01', type_bilan = "sommes_engagees", type_montant = "ttc")
    ...
    if type_bilan == "sommes_engagees"
      if self.commande_solde && (self.depense_equipement_factures.size !=0)
...
#      self.montant_factures('htr', date_start, date_end)
  ...
  def reporting_montant(date_start = '1900-01-01', date_end = '4000-01-01', type = 'facture_htr')
    if type == 'facture_htr'
      ...
      factures = self.depense_equipement_factures.find(:all, options)
      if factures.nil?
        return 0
      else
