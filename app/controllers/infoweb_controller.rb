    ...
    unless text == "OK"
      ...
      render :text => text, :status => '400 Bad Request'
       ...
       if !params[:tutelle].nil?
            ...
            n.numero_ligne_budgetaire as eotp, o.nom as financeur
						FROM lignes l, sous_contrats ss, contrats c left outer join clotures cl on c.id = cl.contrat_id , departements d,
               laboratoires b, notifications n, organisme_gestionnaires o 
						WHERE l.sous_contrat_id  = ss.id
							AND ss.contrat_id = c.id
              AND c.verrou = 'Aucun'
              AND c.id = n.contrat_id
              ...
              AND ss.entite_type = 'Departement'
							...
							AND b.nom=?
						UNION
						SELECT distinct l.id, l.nom, '#{nom_labo}' as 'projet', cl.date_fin_depenses as 'datc', 
            n.numero_ligne_budgetaire as eotp, o.nom as financeur
						FROM lignes l, sous_contrats ss, projets p, contrats c left outer join clotures cl on c.id = cl.contrat_id , 
              laboratoires b, notifications n, organisme_gestionnaires o
						WHERE l.sous_contrat_id  = ss.id
